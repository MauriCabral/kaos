...
public class OrderHistoryController implements Initializable {
    ...
    @FXML private TableColumn<Order, Void> checkboxColumn;
    ...
    @FXML private Button assignDeliveryBtn;
    @FXML private Button closeCashBtn;

    private final IOrderService orderService = new OrderServiceImpl();
    private final IDeliveryService deliveryService = new DeliveryServiceImpl();
    private final ObservableList<Order> orders = FXCollections.observableArrayList();
    private final java.util.Map<Order, Boolean> checkedOrders = new java.util.HashMap<>();
        ...
        checkboxColumn.setCellFactory(param -> new TableCell<Order, Void>() {
...

            {
                checkBox.setOnAction(event -> {
                    ...
                    checkedOrders.put(order, checkBox.isSelected());
            ...
            protected void updateItem(Void item, boolean empty) {
                ...
                if (empty) {
                    ...
                    if (Boolean.TRUE.equals(order.getIsDelivery()) && order.getDelivery() == null) {
                        checkBox.setSelected(checkedOrders.getOrDefault(order, false));
                        setGraphic(checkBox);
    ...
    private void loadOrders() {
        try {
            ...
            updateCheckboxColumnVisibility(allOrders);
    ...
    private void viewOrderDetails(Order order) {
        try {
            ...
            if (fullOrder != null && fullOrder.getOrderDetails() != null) {
                WindowManager.openOrderDetailsWindow(null, fullOrder, false);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void updateAssignButtonState() {
        boolean hasCheckedDeliveryOrders = checkedOrders.entrySet().stream()
                .anyMatch(entry -> Boolean.TRUE.equals(entry.getValue()) &&
                        Boolean.TRUE.equals(entry.getKey().getIsDelivery()) &&
                        entry.getKey().getDelivery() == null);
        assignDeliveryBtn.setDisable(!hasCheckedDeliveryOrders);
    }
    ...
    private void applyFilters() {
        ...
        updateCheckboxColumnVisibility(filtered);
    }
    ...
    private void handleClear() {
        ...
        updateCheckboxColumnVisibility(orders);
    }
    ...
    private void updateCheckboxColumnVisibility(List<Order> ordersToCheck) {
        boolean hasUnassignedDeliveries = ordersToCheck.stream()
                .anyMatch(order -> Boolean.TRUE.equals(order.getIsDelivery()) && order.getDelivery() == null);
        checkboxColumn.setVisible(hasUnassignedDeliveries);
    }

    @FXML
    public void closeCash(ActionEvent actionEvent) {
        try {
            ...
            e.printStackTrace();
            DialogUtil.showError("Error", "No se pudo cerrar caja: " + e.getMessage());
        }
    }
    ...
    private static class DeliveryData {
        ...
        DeliveryData(String name) {
            this.name = name;
            this.cash = BigDecimal.ZERO;
            this.orderCount = 0;
        }

        void addOrder(BigDecimal c) {
            cash = cash.add(c);
            orderCount++;
        }
    }
    ...
    @FXML
    private void assignDeliveryToSelected() {
        List<Order> checkedOrdersList = checkedOrders.entrySet().stream()
                .filter(entry -> Boolean.TRUE.equals(entry.getValue()))
                .map(java.util.Map.Entry::getKey)
                .toList();

        if (checkedOrdersList.isEmpty()) {
            DialogUtil.showWarning("Selección Vacía", "Por favor selecciona al menos un pedido marcando la casilla.");
            return;
        }
        ...
        if (deliveryOrders.isEmpty()) {
            DialogUtil.showWarning("Sin Pedidos de Delivery", "Los pedidos seleccionados no son de delivery o ya tienen delivery asignado.");
            return;
        }
        ...
        if (availableDeliveries.isEmpty()) {
            DialogUtil.showWarning("Sin Deliveries Disponibles", "No hay deliveries disponibles para este local.");
            return;
        }
        ...
        if (selectedDelivery != null) {
            for (Order order : deliveryOrders) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    DialogUtil.showError("Error", "Error al actualizar el pedido " + order.getOrderNumber());
                }
            }

            DialogUtil.showInfo("Éxito", "Delivery asignado a " + deliveryOrders.size() + " pedidos.");
            loadOrders();
            updateAssignButtonState();
        }
    }
    ...
    private Delivery showDeliverySelectionDialog(List<Delivery> deliveries) {
        ...
        deliveryTable.getColumns().addAll(nameCol, phoneCol);
